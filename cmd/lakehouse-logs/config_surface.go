...
const surfaceBinaryName = "lakehouse-logs"
...
// without the flags linked in from VictoriaLogs/VictoriaMetrics packages.
...
func defaultConfigSurface() ([]byte, error) {
	s, err := config.DescribeSurface(surfaceBinaryName, config.ModeLogs, configSurfaceFlags(), applyFlags)
	if err != nil {
		return nil, err
	}
	return s.JSON()
...
//
// Usage: `lakehouse-logs print-default-config`
