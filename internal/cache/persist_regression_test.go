...
func TestRegressionValueCountsMergeAcrossFiles(t *testing.T) {
	...
	if li == nil {		t.Fatal("label info should exist")
	}

	// api should have 50+20=70
	if li.ValueCounts["api"] != 70 {		t.Errorf("api count = %d, want 70", li.ValueCounts["api"])
	}
	// web should stay 30
...
func TestRegressionAddWithValueCountsNilCounts(t *testing.T) {
	...
	if li.ValueCounts["a"] != 2 {		t.Errorf("a count = %d, want 2 (once per occurrence in values)", li.ValueCounts["a"])
	}
...
func TestRegressionAddInitializesValueCounts(t *testing.T) {
	...
	if li.ValueCounts == nil {		t.Error("REGRESSION: ValueCounts should be initialized even by plain Add")
	}
...
func TestRegressionPerTenantCardinality(t *testing.T) {
	...
	if li.PerTenant == nil {		t.Fatal("PerTenant should be initialized")
	}

	if li.PerTenant["tenant1"] != 3 {		t.Errorf("tenant1 cardinality = %d, want 3", li.PerTenant["tenant1"])
	}
...
func TestFuzzyValueCountsAlwaysNonNegative(t *testing.T) {
	...
	if li == nil {		t.Fatal("label should exist")
	}

	for v, c := range li.ValueCounts {		if c < 0 {
