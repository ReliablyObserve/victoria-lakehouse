...
func TestInteg_bloomFilterFiles_WithIndex(t *testing.T) {
	...
	loader := func(_ context.Context, partition string) (*bloomindex.Index, error) {
		// Tenant-isolated partition (manifest.ExtractTenantPartition: the full key
		// dir incl any tenant prefix) — matches how the pre-filter now groups files
		// and how PersistDirty keys _bloom.bin.
