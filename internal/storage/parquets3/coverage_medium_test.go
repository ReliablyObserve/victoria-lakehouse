...
func TestMedium_detectConstantColumns(t *testing.T) {
	...
	t.Run("single constant column", func(t *testing.T) {
		...
		// service.name is ByteArray — even when all rows in the page
		// share the value, the column-index min/max may be truncated
		// per Apache Parquet's PageIndex spec. We never treat ByteArray
		// columns as constants; callers fall through to data-page reads
		// which return the full, untruncated value.
		if len(constants) != 0 {
