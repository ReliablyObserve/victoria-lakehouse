...
func parquetValueToInterface(v parquet.Value) any {
	// A NULL cell (an `optional` column the row never set) has no Kind, so the
	...
	// unset dedicated/slot column on every cold row. VictoriaLogs treats an
	// empty value as a non-existing field ("skip empty fields, since they equal
	// to non-existing fields" — app/vlselect/logsql/logsql.go), so mapping NULL
	// to "" makes the cold row carry exactly the fields the hot row carries.
	// Sibling guard of parquetValueToString / parquetValueToAny / valueToString.
	if v.IsNull() {
		return ""
	}
	switch v.Kind() {
