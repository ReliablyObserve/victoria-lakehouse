...
func TestMemLeak_BuildPushDownFilter(t *testing.T) {
	...
	const iterations = 10000
	for i := 0; i < iterations; i++ {
		q := fmt.Sprintf(`service.name:="svc-%d" severity_text:="ERROR"`, i%50)
		_ = buildPushDownFilter(q, reg)
	}
...
// --- queryColumns extraction ---

func TestMemLeak_QueryColumns_Extraction(t *testing.T) {
	// Warm up
	for i := 0; i < 100; i++ {
		q := fmt.Sprintf(`service.name:="svc-%d" severity_text:="INFO"`, i%10)
		reg := schema.NewRegistry(schema.LogsProfile)
...
func TestMemLeak_ExtractLogLabels(t *testing.T) {
	rows := make([]schema.LogRow, 50)
	for i := range rows {
	...
	maxAllowed := int64(10 * 1024 * 1024)
	if growth > maxAllowed {
		t.Errorf("extractLogLabels memory grew by %d bytes over %d iterations (max allowed %d)", growth, iterations, maxAllowed)
	}
}
		...
		for h := 0; h < 24; h++ {
			...
			for f := 0; f < 5; f++ {
				m.AddFile(partition, manifest.FileInfo{
					Key:  fmt.Sprintf("logs/%s/file-%d.parquet", partition, f),
					Size: 1024,
				})
			}
		}
...
func TestMemLeak_LabelIndex_Updates(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		idx.Add("severity_text", []string{"INFO", "ERROR", "WARN"})
	}
	mlForceGC()
...
func TestMemLeak_ExtractLogBloomValues(t *testing.T) {
	...
	maxAllowed := int64(10 * 1024 * 1024)
	if growth > maxAllowed {
		t.Errorf("extractLogBloomValues memory grew by %d bytes over %d iterations (max allowed %d)", growth, iterations, maxAllowed)
	}
}
