...
func (s *Storage) GetFieldValues(ctx context.Context, tenantIDs []logstorage.TenantID, q *logstorage.Query, fieldName string, limit uint64) ([]logstorage.ValueWithHits, error) {
	...
	//
	// Nor is the pmeta catalog: it holds each partition's value SET, with no
	// counts, so it answered every value with hits=1 while VictoriaLogs returns
	// the number of rows carrying it. Exact per-file counts come from the
	// manifest's label aggregates instead (collectFieldValues).
