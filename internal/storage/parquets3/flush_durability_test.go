...
// the batch twice.
func TestFlush_SizeTriggeredFlushNeverBlocksTheInsert(t *testing.T) {
	u := &faultyUploader{block: make(chan struct{}), started: make(chan struct{}, 1)}
	bw, m := durabilityWriter(t, u)
	...
	go func() {
		bw.AddLogRows(rowsAt(time.Date(2026, 5, 3, 14, 0, 0, 0, time.UTC), 20, 0))
		close(returned)
	}()
	select {
	case <-returned:
...
func TestFlush_ARetryOverwritesTheSameObject(t *testing.T) {
	...
	bw, m := durabilityWriter(t, u)
	bw.AddLogRows(rowsAt(time.Date(2026, 5, 3, 14, 0, 0, 0, time.UTC), 50, 0))
