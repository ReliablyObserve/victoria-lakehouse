...
// makeTestParquetData creates a small Parquet file from log rows for testing.
func makeTestParquetData(t *testing.T) ([]byte, string) {
	t.Helper()
	rows := []schema.LogRow{
		{
...
func TestCacheOnFlush_StoresColumnChunks(t *testing.T) {
	...
	// Check that known columns are cached.
	expectedCols := []string{"body", "severity_text", "service.name", "timestamp_unix_nano"}
