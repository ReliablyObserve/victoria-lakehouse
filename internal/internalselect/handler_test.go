...
func TestHandler_StreamEndpoints(t *testing.T) {
	...
	for _, ep := range endpoints {
		...
		if len(data) < 8 {
			t.Errorf("%s: data too short: %d bytes", ep.path, len(data))
			continue
		}
...
func TestHandler_ProtocolVersionMismatch(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/internal/select/query?version=wrong"+
		"&tenant_ids=[]&timestamp=0&query=*"+
		"&disable_compression=false&allow_partial_response=false&hidden_fields_filters=[]", nil)
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
	...
	if !bytes.Contains(rec.Body.Bytes(), []byte("unexpected protocol version")) {
		t.Errorf("expected protocol version error, got: %s", rec.Body.String())
	}
}
