...
// NewRewriter creates a Rewriter with the given pool, key prefix, and row group size.
// If rowGroupSize <= 0 it defaults to 10000.
func NewRewriter(pool RewriterPool, prefix string, rowGroupSize int) *Rewriter {
	if rowGroupSize <= 0 {
		rowGroupSize = 10000
	}
	return &Rewriter{
...
func (r *Rewriter) RewriteFile(ctx context.Context, key string, tombstones []Tombstone) (*RewriteResult, error) {
	...
	// Filter rows: keep those not matching any tombstone.
	...
	removed := int64(len(rows)) - int64(len(kept))
	...
	// If all rows removed, delete old file and return without uploading a new one.
	if len(kept) == 0 {
	...
	// Write kept rows to a new Parquet file.
	...
	// Build new key: {prefix}{partition}/{uuid[:8]}.parquet
...
func logRowToMap(row *schema.LogRow) map[string]string {
	...
	// Merge resource and log attributes.
