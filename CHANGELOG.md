...
- **Multi-instance gossip now actually runs.** `Storage.RefreshDiscovery` — the only path that resolves the peer ring into the stats + tenant-alias `SyncPusher`s and the Phase D fleet-metadata gossip — was never invoked at runtime, so `GetPeers()` always returned empty and a multi-node deployment silently never gossiped (each node only ever saw itself + a stale S3-snapshot node). A `startPeerDiscovery` loop now calls it once at startup and every `discovery.peer_refresh_interval` (default 30s) when a peer headless service is configured, and peer discovery is made non-fatal to a storage-node/partition-list hiccup so it can't be starved. Single-instance behavior is unchanged (no-op without a peer service). Both modules. Found by standing up a 3-node cluster (`deployment/docker/docker-compose-cluster.override.yml`).
