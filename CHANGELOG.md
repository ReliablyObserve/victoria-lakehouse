...
- **A permanent delete could make the rows it was supposed to KEEP disappear — or make the rows it deleted reappear.** The background rewriter wrote a filtered replacement Parquet object and deleted the original, but nothing ever told the manifest. The manifest kept pointing at the deleted key, which failed in two opposite directions depending on whether the tombstone survived: with the tombstone present, the query path's 404 recovery skipped the missing key and the kept rows vanished from every result; with the tombstone lost, the same path synthesised `RowCount` blocks from the stale manifest entry and the deleted rows came back in counts and hit totals. The replacement, meanwhile, was in nobody's manifest, so the orphan sweep deleted it once it passed `orphan_ttl` — taking the kept rows with it permanently. Rewrites are now two-phase: the replacement is uploaded, the manifest swaps the old key for the new one in a single atomic step (`Manifest.ReplaceFile`), and only then is the superseded object deleted. Row counts, time bounds, raw/column byte accounting and per-value label aggregates are recomputed from the kept rows rather than inherited, so `stats count() by (field)` — which is answered from manifest metadata without opening a file — stops counting deleted rows. The rewriter now refuses to run at all when no manifest is wired in (counted as `lakehouse_delete_rewrite_skipped_no_manifest_total`), since rewriting blind is what destroyed data; a tombstone whose every file has been rewritten is retired instead of staying in `Active()` forever; and a key that a crash left already superseded is recognised rather than retried until the end of time. Pinned by a crash matrix that injects a fault at each of prepare/publish/commit, a property test over 24 seeded random delete/rewrite/restart sequences, and a reusable `internal/testutil/storageinvariants` package asserting that the manifest key set matches the bucket, that summed `RowCount` equals a full scan, and that no fully reaped tombstone stays active. No perf delta on the read path; a rewrite now performs one extra manifest swap.

- **A hide-mode delete was undone by any ungraceful restart, and never applied to field pickers or compaction.** Tombstones reached local disk only from `runShutdown`, `SyncToS3` had no caller at all, and the S3 key prefix was built with a doubled slash (`logs//_tombstones/`) so the startup `LoadFromS3` listed a prefix nothing was ever written to. A `SIGKILL`, an OOM kill or a node failure therefore silently un-deleted everything an operator had hidden. Tombstones are now written through on every change — disk synchronously before the API returns, S3 in the same call with a retry queue drained by the scheduler tick and at shutdown — and startup restores the *union* of both copies, resolving per-record conflicts towards the one with the most rewrite progress. Separately, tombstones were applied only on the row-fetch path: `field_values`, `streams` and `stream_ids` still enumerated a deleted value (so it stayed selectable in Grafana dropdowns), `field_names` still reported hit counts that included deleted rows, and compaction copied tombstoned rows verbatim into each new merged object. The enumeration paths now apply the tombstone predicate — giving up the in-RAM catalog and label-index fast paths while a tombstone overlaps the window, counted as `lakehouse_delete_fields_scan_fallback_total` — `field_names` reports unknown (`0`) hit counts rather than wrong ones, and compaction drops tombstoned rows before deriving any manifest metadata and marks the merged source keys reaped. A boot-time self-check compares the restored tombstones against the restored manifest and counts every disagreement by kind (`lakehouse_delete_startup_inconsistencies_total`). Both modules. Perf: no change while no tombstone overlaps a query's window; while one does, the affected enumeration endpoints fall back to a column-projected row scan (correctness over latency), and the LogsQL filter behind a tombstone is now parsed once and cached instead of re-parsed per row, which makes the row filter itself substantially cheaper than before.
