...
- **pmeta `retire-sidecar-writes` — stop writing the legacy `_file_metadata.json` sidecar once the facet read-path is trusted (`-lakehouse.pmeta.retire-sidecar-writes`, off by default, both modules).** The back half of the consolidation: with the read-flip serving file metadata from the in-RAM fileMetaFacet (warmed from the bundle), the per-partition `_file_metadata.json` write is skipped at flush — removing one S3 PUT per partition per flush. The Parquet **footer** remains the cold-restart fallback (`WarmMetadata` Phase 3), so this is safe, and it is **reversible** (clear the flag → the sidecar resumes; no data migration). Requires `--pmeta`. `TestInteg_PmetaRetire_SkipsFileMetaSidecar` asserts the sidecar is not written yet the metadata is still served from the facet. The same flag also retires the redundant **per-file `.bloom`** write (the pmeta bloom facet covers `checkFileBloom`); the partition **`_bloom.bin`** is deliberately KEPT because the OR-branch query path (`bloomFilterFilesByOrBranches` → `bloomCache`) still reads it and is not yet flipped to the facet — so OR-query pruning is unaffected. `TestBloomObserver_RetireFileBloom`. (Flipping the OR-branch path + the hard code/doc cleanup follow once this bakes.)
