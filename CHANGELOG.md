...
- **Benchmark harness now validates every timed response before counting its latency.** Previously the harness timed every request but fetched the comparable result (row/scalar count) only once, outside the loop — a system answering fast with an error, an empty result, or the wrong rows still earned a latency sample (this was exactly the VT-empty-cells failure mode found while recording the baseline above). `scripts/bench/run.sh` now captures and validates every iteration's response — HTTP status, parseability, non-empty (unless the query is a documented miss scenario), and identical to the cell's first valid result (no flapping) — and drops invalid iterations from p50/p95/p99 instead of counting them; `scan` queries are compared by a sha256 hash of their sorted row-key set (`_msg` for logs, `trace_id:span_id` for traces), not just row count, so a same-count-but-different-rows answer is still caught. `scripts/bench/report.py` cross-checks each engine's result against its signal's baseline and shows a `k/N valid` column per system. Re-baselined both signals with validation on: `bench-results/baseline-2026-09/run-baseline-v3.{json,md,log}` (60/64 cells valid; the 4 exceptions are `scan`/24h on both signals, a genuine baseline flapping finding — VictoriaLogs/VictoriaTraces return a different subset of rows on repeated identical unordered top-N queries once the result exceeds the `limit 1000` cap — not a harness defect) supersedes v1/v2 as the perf-gate reference. New tests: `scripts/bench/tests/test_report_validity.py`, `scripts/bench/tests/extract_result_test.sh`.
