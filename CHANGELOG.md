...
- **`FuzzParseFooterBytes` nightly hang (`internal/storage/parquets3`).** The nightly fuzz job for `ParseFooterBytes` has hung/died on every `main` run since at least 2026-09-08 (exec/sec collapsing to ~2-5/s before the worker died, no panic text captured). Root cause, reproduced locally within the first 120s fuzz run: `ParseFooterFromBytes` fed arbitrary footer bytes straight into `parquet-go`'s `OpenFile`, which independently re-parses the trailing 8 bytes of that buffer for a footer length and allocates a buffer sized directly from it (`make([]byte, footerSize)`) — a raw little-endian `uint32` fully controlled by the input, up to ~4 GiB, driving a multi-GiB allocation per call under fuzzing/adversarial input. The same investigation also turned up a second, distinct bug: a crafted footer whose thrift-decoded schema carries a negative `SchemaElement.NumChildren` panics inside `parquet-go`'s column loader (`make([]*Column, numChildren)` with `numChildren < 0`) instead of erroring. `ParseFooterFromBytes` now (1) validates the declared footer length against the actual buffer size and a 16 MiB sane maximum before ever calling into `parquet-go`, rejecting a malformed/malicious length immediately instead of allocating from it, and (2) recovers any panic from the vendored decoder and converts it to an error, so a crafted or corrupted footer can never crash the caller. Fixed in both the logs and traces modules (duplicated function). Regression: the minimized crasher is committed as a fuzz seed corpus entry plus two unit tests (`TestParseFooterFromBytes_RejectsAbsurdFooterLength`, `TestParseFooterFromBytes_RecoversFromDecoderPanic`) in both modules. Verified: `FuzzParseFooterBytes` runs clean for 120s post-fix (~6.4M execs, no crashes, exec/sec never drops below ~10K/s); no perf delta expected on valid files since the new checks are on the already-known footer length, not the data path.
