...
- **Parquet compression step 1: delta-encoded timestamps + standard dictionary encoding on low-cardinality strings (both modules).** Per the approved research (`docs/architecture/parquet-compression-research.md`): `DELTA_BINARY_PACKED` on `timestamp_unix_nano`/`start_time_unix_nano` and `RLE_DICTIONARY` (the `dict` struct tag — parquet-go does not dictionary-encode by default) on 17 low-cardinality string columns; high-cardinality columns (body, ids, urls) deliberately untouched. One schema edit covers both modules, the compactor, and the Option-B export. Files remain standard-parquet (encodings verified readable by pyarrow AND duckdb in the research harness). Measured on REAL stack data via the new `scripts/bench/compression_ab` A/B harness (identical rows, baseline vs tagged schema): **−2.2% (zstd-default) / −2.4% (zstd-best)** — honest small win; the corpus is dominated by the untouched `body` column. The same harness measures every subsequent compression step (sorting is next and the expected big win).
