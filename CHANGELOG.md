...
- **Helm chart defaults follow the code defaults.** Two values change what chart deployments run:
  - `lakehouseConfig.insert.flush_interval`: `30s` → `1m`, the code default (the chart had set 30s believing it matched the code).
  - `lakehouseConfig.insert.compression_level`: `7` → `3`: fresh writes use zstd Default and compaction still recompresses older files at levels 7 and 11. Set `compression_level: 7` to keep the previous behavior.

  Values that never took effect now show the code default, with no runtime change: `query.max_files_per_query` `500` → `0`, `cache.warmup_max_files` `500` → `0`, `cache.warmup_concurrency` `4` → `0` and `pmeta.always_sketch_fields` `[trace_id, span_id]` → `[]` (the binaries do not read these from the config file; trace_id and span_id are always sketched); the S3 read-ahead, coalescing, read-buffer and planned-fetch sizes `0` → their byte defaults (0 meant "use the default"); the `*_scaling` policies `fixed` → empty (equivalent); `prefetch.max_concurrent` `4` → `8`, `prefetch.max_queue` `64` → `128`, `startup.max_resync_time` `2m` → `10m` and `shutdown.flush_timeout` `15s` → `30s` (no binary reads these); `logs.config.bloom_columns` gains `trace_id`, which the writer always bloom-indexes anyway. Kept as documented deployment overrides: `query.file_workers: 8` (code 64, which OOMed 2 GiB pods on wildcard queries), `insert.buffer_engine: logstore` (code `buffer`) and the S3 download request/limit pair `4`/`16` (the same effective bounds as the code default, without its deprecation warning). The schema defaults match the code (`s3.projected_fetch_mode` is `window`, not `planned`), the dead `circuit_breaker` values are removed, the config map now renders the `shutdown` section (`persist_timeout` is read by both binaries) and the `jaeger_enabled` template fallback is `true`, like the binary. Upgrading rolls the pods because the config checksum changes.
