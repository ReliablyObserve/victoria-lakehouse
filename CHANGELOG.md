...
- **Benchmark harness now validates every timed response before counting its latency, including membership-validated truncated scans.** Previously the harness timed every request but fetched the comparable result (row/scalar count) only once, outside the loop — a system answering fast with an error, an empty result, or the wrong rows still earned a latency sample (this was exactly the VT-empty-cells failure mode found while recording the baseline above). `scripts/bench/run.sh` now captures and validates every iteration's response — HTTP status (curl no longer suppresses real 4xx/5xx into "000"), parseability, non-empty (unless the query is a documented miss scenario), and identical to the cell's first valid result (no flapping) — and drops invalid iterations from p50/p95/p99 instead of counting them; a `scan`'s content is compared by a sha256 hash of its sorted row-key set (`_msg` for logs, `trace_id:span_id` for traces), not just row count. VictoriaLogs documents that `limit N` without `sort` returns rows in arbitrary order once more than N rows match, so identity can never hold for a truncated scan — validity there is now membership (every returned row is a member of a per-cell reference window, one untimed unlimited request per system) plus cardinality (exactly `limit` rows), not identity, and the reference window's row-set key is NUL-delimited (not newline-delimited) so a real log message containing an embedded newline (e.g. a stack trace) can't fragment into bogus extra keys. `scripts/bench/report.py` cross-checks each engine's result (or, for `scan`, its reference-window row count/hash) against its signal's baseline, shows a `k/N valid` column per system, and only counts a ClickHouse speedup when the paired LH cell is itself valid; the per-cell log line always shows `valid=<k>/<N>` next to `p95=`. Re-baselined both signals three times: `run-baseline-v3.{json,md,log}` (60/64 cells valid; the 4 exceptions were `scan`/24h flapping under the old identity rule), `run-baseline-v3.1.{json,md,log}` (64/64 with the membership rule), and `run-baseline-v3.2.{json,md,log}` — every request in one (signal, query, range, latency) cell now shares ONE set of window bounds from a single `time.time()` call instead of each system computing its own a few (milli)seconds apart, so `report.py` requires EXACT equality between baseline and each engine instead of a 5% tolerance; a group-by result (`count_by_service`, `high_card`) is hashed by its sorted (group, count) pairs instead of reduced to `sum(n)`, so a same-total-different-groups divergence is caught; ClickHouse's `scan` is now requested as `FORMAT JSONEachRow` with its columns aliased to `_msg`/`trace_id`/`span_id`, so it goes through the same extractor as VL/VT/LH and gets a real `window_hash` instead of being validated by row count alone; a parse failure now triggers on ANY unparseable line, not just when every line fails; and the perf gate compares LH's own p90+p50 against a run, not p95 (which is literally the single max sample at `--iterations 20`) or the LH/baseline ratio. v3.2: LH 64/64 rows valid, ClickHouse 63/64 (one genuine, extremely minor, ClickHouse-specific `GROUP BY` cardinality difference at ~6,557 groups — a real finding the new group-by hashing surfaced, not a harness defect) — supersedes v3.1/v3/v2/v1 as the perf-gate reference. New tests: `scripts/bench/tests/test_report_validity.py`, `scripts/bench/tests/extract_result_test.sh`, `scripts/bench/tests/scan_membership_test.sh`, `scripts/bench/tests/measure_query_test.sh` (116 checks total, up from 0 before this work).
