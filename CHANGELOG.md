...
- **Storage Overview metadata footprint (Phase B).** A new **Metadata footprint** section shows where metadata lives: pmeta RAM (this node), disk cache (this node), and the cluster-wide on-S3 `_meta/` total (pmeta bundles + snapshots + sidecars). `/stats/overview` exposes `meta_resident_bytes` / `meta_disk_bytes` / `meta_s3_bytes`; the S3 figure is swept via `ClientPool.PrefixBytes` on the manifest-refresh cadence (not per request). Both modules.
