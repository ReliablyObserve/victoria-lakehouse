...
func TestParity_PipesExtended(t *testing.T) {
	cases := []ParityCase{
		...
		// Log rows carry no `duration` field (cmd/datagen writes durations
		// into the message text only); severity_number is the corpus's
		// numeric column, so the math pipe reads that.
		{Name: "math_duration", Endpoint: statsEndpoint(), Params: map[string]string{"query": "* severity_number:* | math severity_number / 1000 as severity_scaled | stats count() rows"}, Compare: CountEqual},
