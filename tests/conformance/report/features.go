...
// Write appends fixed text, so fmt.Fprintf can render into a Document.
func (d *Document) Write(p []byte) (int, error) {
	d.parts = append(d.parts, docPart{text: string(p)})
	return len(p), nil
}

// WriteString appends fixed text.
...
func RenderFeatures(set *registry.FeatureSet, reg *registry.Registry, changelog *registry.Changelog, outDir string) *Document {
	...
	fmt.Fprintf(b, "Legend: %s shipped and covered — the catalog links at least one regression test (the file and the test function are checked to exist; the catalog does not run them) %s · %s shipped, declared only — every linked row is still `pending` and no test is linked · %s in progress · %s planned (documented, not implemented)\n\n",
	...
	if gaps == 0 {
		fmt.Fprintf(b, "None: every shipped feature links at least one test or a registry row that executes.\n")
	}
	b.WriteString("\n")
	...
	for _, area := range registry.Areas {
		...
		for _, f := range features {
			...
			fmt.Fprintf(b, "`%s` · status: %s", f.ID, f.Status)
			switch {
			case f.Since != "":
