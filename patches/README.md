...
| `vl-flush-sink.go.src` | `lib/logstorage/vl_flush_sink.go` (new file) | Adds the exported `FlushSink func(FlushRowsIterator)` hook + `FlushRowsIterator` type and the package-local `convertPartsForFlush` / `(*datadb).flushPartsViaSink` helpers. Enables the cold tier's Option B (buffer = a real `logstorage.Storage`; Parquet→S3 flush is a conversion from its in-memory parts). Inert while `FlushSink == nil`. See `docs/architecture/buffer-queryable-store-design.md`. |
| `vl-flush-sink-hook.patch` | `lib/logstorage/datadb.go` | In `mustMergePartsInternal`, diverts the in-memory-part→disk seam (`dstPartType != partInmemory`) to `flushPartsViaSink` when `FlushSink != nil`. Guarded by the nil check, so stock VL behavior is unchanged until the cold tier registers a sink. |
