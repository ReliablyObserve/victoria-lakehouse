...
// TestConfigSurfaceGolden pins what `lakehouse-traces print-default-config`
...
func TestConfigSurfaceCoversEveryLakehouseFlag(t *testing.T) {
	...
	if s.Binary != "lakehouse-traces" || s.Mode != config.ModeTraces {
		t.Fatalf("surface names %s/%s, want lakehouse-traces/traces", s.Binary, s.Mode)
	}
...
func TestCompactionEnabledResolution(t *testing.T) {
	cases := []struct {
		...
		{name: "signal profile in the file", file: "traces:\n    profile: max-cost-savings", want: false},
		{name: "role profile in the file for this role", file: "traces:\n    insert:\n      profile: dev", role: config.RoleInsert, want: false},
		{name: "role profile in the file for another role", file: "traces:\n    insert:\n      profile: dev", role: config.RoleSelect, want: true},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			cfg, err := config.LoadWithMode(path, config.ModeTraces, c.role)
			if err != nil {
				t.Fatal(err)
			}
