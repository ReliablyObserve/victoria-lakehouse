...
func TestDetectConstantColumns_EmptyWant(t *testing.T) {
	dir := t.TempDir()
	rows := []pushdownTestRow{{TimestampUnixNano: 1000, SpanName: "a", SeverityText: "info", ServiceName: "svc"}}
	path := writePushdownTestParquet(t, dir, rows)
	f := openTestParquet(t, path)
	rgs := f.RowGroups()
	if len(rgs) == 0 {
...
// --- readRowGroupColumnar with bitmap ---
...
func TestReadRowGroupColumnar_WithBitmap(t *testing.T) {
	...
	db := readRowGroupColumnar(f, rgs[0], wantCols, reg, 0, int64(2000), bitmap, nil, nil)
	if db == nil {
...
func TestReadRowGroupColumnar_NoTimestamp(t *testing.T) {
	...
	db := readRowGroupColumnar(f, rgs[0], wantCols, reg, 0, int64(9999), nil, nil, nil)
	if db == nil {
		t.Fatal("expected non-nil DataBlock")
	}
}
...
func TestReadRowGroupColumnar_AllFilteredByTime(t *testing.T) {
	...
	db := readRowGroupColumnar(f, rgs[0], wantCols, reg, 9000, 9999, nil, nil, nil)
	if db != nil {
...
func TestReadRowGroupColumnar_EmptyWantCols(t *testing.T) {
	...
	reg := schema.NewRegistry(schema.TracesProfile)
	db := readRowGroupColumnar(f, rgs[0], nil, reg, 0, 9999, nil, nil, nil)
	if db != nil {
		t.Error("expected nil for empty wantCols")
	}
}

// --- rowGroupMatchesFilter additional branches ---
