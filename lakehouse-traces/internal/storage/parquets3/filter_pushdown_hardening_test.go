...
// pushdownTestRow is a minimal row type for pushdown filter tests.
// Uses SpanName instead of Body for the traces module.
...
func TestBuildPushDownFilter_NegatedPredicateSkipped(t *testing.T) {
	...
	pdf := buildPushDownFilter(`!cloud.region:="us-east-1"`, reg)
	if pdf != nil {
		t.Error("expected nil filter for negated predicate, got non-nil")
	}

	pdf = buildPushDownFilter(`NOT cloud.region:="us-east-1"`, reg)
	if pdf != nil {
...
func TestRowGroupMatchesFilter_DictionaryPrefixHit(t *testing.T) {
	dir := t.TempDir()
	rows := []pushdownTestRow{
		{TimestampUnixNano: 1000, SpanName: "a", SeverityText: "info", ServiceName: "prod-api"},
		{TimestampUnixNano: 2000, SpanName: "b", SeverityText: "warn", ServiceName: "prod-web"},
	}
	path := writePushdownTestParquet(t, dir, rows)
	f := openTestParquet(t, path)
	rgs := f.RowGroups()
