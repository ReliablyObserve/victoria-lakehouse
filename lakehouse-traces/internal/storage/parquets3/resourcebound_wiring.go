...
// Mirror of internal/storage/parquets3/resourcebound_wiring.go in the
// logs module. Keeps both modules' bound construction + wiring in
// lockstep so operators see the same K8s-style request/limit/usage
// signal regardless of which Lakehouse binary they query.
//
// The bound foundation (internal/resourcebounds) and metric definitions
// (internal/metrics) are imported from the root module via the
// `replace github.com/ReliablyObserve/victoria-lakehouse => ../`
// directive in lakehouse-traces/go.mod — there is one bound type and
// one set of Prometheus metrics for both binaries.
...
// newS3DownloadsBound constructs the K8s-style request/limit Bound for
// S3 download concurrency. Same construction as the logs module — the
// runtime wiring in the traces module is currently metric-exposure-only
// because the traces getFileData path does not have the channel-based
// admission point that the logs module wires through; the bound's
// request/limit gauges still surface the operator contract.
//
// Defaults: request=4, limit=16.
...
// resource surfaces that operator dashboards observe. Mirror of the
// logs module struct.
...
// newQueryFileWorkersBound constructs the file-workers bound. Same
// shape as the logs module.
...
// newCacheMemoryBound — mirror of the logs module bound.
...
// newSmartCacheDiskBound — mirror of the logs module bound.
...
// newQueryMaxRowsBound — mirror of the logs module bound.
...
// parseSizeOr0 — mirror of the logs module helper.
...
// newResourceBoundSet constructs all 5 bounds and populates the
// per-surface request/limit gauges at startup. Mirror of the logs
// module factory.
