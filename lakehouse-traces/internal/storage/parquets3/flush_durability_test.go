...
// rowsAt returns n spans one second apart from base, of one tenant.
func rowsAt(base time.Time, n int, account uint32) []schema.TraceRow {
	rows := sampleTraceRows(n, base)
	for i := range rows {
...
// writes them — once.
func TestTraceFlush_FailedUploadKeepsItsRows(t *testing.T) {
	...
	if got := len(bw.BufferedTraceRows(0, base.Add(time.Hour).UnixNano())); got != 50 {
	...
	if bw.BufferedRows() != 0 || len(bw.BufferedTraceRows(0, base.Add(time.Hour).UnixNano())) != 0 {
...
func TestTraceFlush_OnlyTheFailedTenantGroupIsRetried(t *testing.T) {
	...
	base := time.Date(2026, 5, 3, 14, 0, 0, 0, time.UTC)
	bw.AddTraceRows(append(rowsAt(base, 30, 0), rowsAt(base.Add(time.Minute), 20, 7)...))
...
func TestTraceFlush_PartitionsNotReachedBeforeTheDeadlineArePutBack(t *testing.T) {
	...
	for h := 0; h < 12; h++ { // twelve partitions
		bw.AddTraceRows(rowsAt(base.Add(time.Duration(h)*time.Hour), 10, 0))
	}
...
func TestTraceFlush_RowsStayVisibleWhileInFlight(t *testing.T) {
	u := &faultyUploader{block: make(chan struct{}), started: make(chan struct{}, 1)}
	bw, m := durabilityWriter(t, u)
	base := time.Date(2026, 5, 3, 14, 0, 0, 0, time.UTC)
	bw.AddTraceRows(rowsAt(base, 25, 0))
	...
	if got := len(bw.BufferedTraceRows(0, window)); got != 25 {
	...
	if got := len(bw.BufferedTraceRows(0, window)); got != 0 {
...
func TestTraceFlush_RandomFailuresUnderConcurrentInsertsLoseNothing(t *testing.T) {
	...
	for wi := 0; wi < writers; wi++ {
		...
		go func(wi int) {
			...
			for b := 0; b < batches; b++ {
				...
				bw.AddTraceRows(rowsAt(at, perBatch, uint32(wi%2)))
			}
		}(wi)
	}
...
// rows are written.
func TestTraceCanWriteData_TooMuchUnwrittenDataIs429(t *testing.T) {
	var failing atomic.Bool
	failing.Store(true)
	u := &faultyUploader{fail: func(string) error {
		if failing.Load() {
	...
	base := time.Date(2026, 5, 3, 14, 0, 0, 0, time.UTC)
	bw.AddTraceRows(rowsAt(base, 200, 0))
	_ = bw.FlushAll(context.Background())
...
func TestTraceCanWriteData_ProbeIsReusedAndAnUnwritableStoreIs503(t *testing.T) {
...
func TestTraceStop_RowsTheFinalFlushCannotWriteAreCounted(t *testing.T) {
	u := &faultyUploader{fail: func(string) error { return errPutFailed }}
	bw, _ := durabilityWriter(t, u)
	bw.cfg.FlushInterval = time.Hour
	bw.Start()
	bw.AddTraceRows(rowsAt(time.Date(2026, 5, 3, 14, 0, 0, 0, time.UTC), 12, 0))
