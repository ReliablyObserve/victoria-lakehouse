...
func TestTierTransition_QueryCorrectness_AllTiers(t *testing.T) {
	...
	ttFileKeys := make([]string, numFiles)
	for f := 0; f < numFiles; f++ {
		ttFileKeys[f] = fmt.Sprintf("%s/file%d.parquet", partition, f)
	}
	...
	for f := 0; f < numFiles; f++ {
		for rg := 0; rg < 5; rg++ {
			rgKeys = append(rgKeys, bloomindex.PerRGKey(ttFileKeys[f], rg))
		}
	}
	...
	for _, k := range tier1Result {
		...
		if ok && fk == ttFileKeys[targetFile] {
			tier1Found = true
			break
		}
	}
	...
	tier2Result := tier2.MayContain(ttFileKeys, "trace_id", targetTrace)
	if !containsKey(tier2Result, ttFileKeys[targetFile]) {
