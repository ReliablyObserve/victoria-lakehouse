...
// Mirror of internal/storage/parquets3/pure_buffer_tombstone_test.go.
//
...
func TestRunQuery_PureBufferWindowHonoursTombstones(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		...
		lr.MustAdd(logstorage.TenantID{}, now, []logstorage.Field{
			...
			{Name: "trace_id", Value: "trace-" + strconv.Itoa(i)},
		}, 1)
	}
	bs.MustAddRows(lr)
	...
	if got := count(); got != 6 {
		t.Fatalf("fixture: want 6 buffered spans counted before any delete, got %d", got)
	}
	...
	if got := count(); got != 4 {
		t.Fatalf("count over the unflushed window = %d, want 4: the 2 tombstoned spans must not be counted", got)
	}
}
