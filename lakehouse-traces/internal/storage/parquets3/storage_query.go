...
func (s *Storage) queryBufferBridge(ctx context.Context, startNs, endNs, watermarkNs int64, q *logstorage.Query, tenantIDs []logstorage.TenantID, filteredWriteBlock logstorage.WriteDataBlockFunc) {
	...
	// the row→block conversion re-checks every row's account/project — belt and
	// braces, so a peer that answers without scoping cannot leak into this
	// answer. Twin of internal/storage/parquets3/storage_query.go.
	scope := scopeFor(ctx, tenantIDs)
	switch s.cfg.Mode {
...
func (s *Storage) queryFile(ctx context.Context, fi manifest.FileInfo, startNs, endNs int64, queryStr string, pipeFields []string, writeBlock logstorage.WriteDataBlockFunc) error {
	...
	// silently skipping the very row groups that hold the match. Symptom (logs
	// twin): `trace_id:=X | stats count()` returned 0 while the `trace_id:=X`
	// retrieval (projectedCols==nil → full download) returned the rows. The
	// file-level pmeta bloom has already pruned files; the per-row-group
