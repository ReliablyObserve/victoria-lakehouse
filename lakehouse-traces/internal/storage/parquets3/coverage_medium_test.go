...
// ---------------------------------------------------------------------------
// 1. rowGroupFullyInRange -- SKIPPED: function does not exist in traces module.
//    The traces module does not implement rowGroupFullyInRange.
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// 2. prewhereFilter (prewhere.go ~12) -- covered
// ---------------------------------------------------------------------------
...
func TestMedium_prewhereFilter(t *testing.T) {
	...
	rows := []pushdownTestRow{
		{TimestampUnixNano: 1000, SpanName: "a", SeverityText: "info", ServiceName: "alpha"},
		{TimestampUnixNano: 2000, SpanName: "b", SeverityText: "warn", ServiceName: "beta"},
		{TimestampUnixNano: 3000, SpanName: "c", SeverityText: "error", ServiceName: "alpha"},
		{TimestampUnixNano: 4000, SpanName: "d", SeverityText: "info", ServiceName: "gamma"},
	}
...
// ---------------------------------------------------------------------------
// 3. detectConstantColumns (constant_columns.go ~15) -- covered
// ---------------------------------------------------------------------------
...
func TestMedium_detectConstantColumns(t *testing.T) {
	...
	t.Run("single constant column", func(t *testing.T) {
		rows := []pushdownTestRow{
			{TimestampUnixNano: 1000, SpanName: "a", SeverityText: "info", ServiceName: "alpha"},
			{TimestampUnixNano: 2000, SpanName: "b", SeverityText: "warn", ServiceName: "alpha"},
			{TimestampUnixNano: 3000, SpanName: "c", SeverityText: "error", ServiceName: "alpha"},
		}
		path := writePushdownTestParquet(t, dir, rows)
		f := openTestParquet(t, path)
	...
	t.Run("varying column not detected", func(t *testing.T) {
		...
		rows := []pushdownTestRow{
			{TimestampUnixNano: 1000, SpanName: "a", SeverityText: "info", ServiceName: "alpha"},
			{TimestampUnixNano: 2000, SpanName: "b", SeverityText: "warn", ServiceName: "beta"},
			{TimestampUnixNano: 3000, SpanName: "c", SeverityText: "error", ServiceName: "gamma"},
		}
...
// ---------------------------------------------------------------------------
// 4. stripTimePredicates (filter.go ~51) -- cover uncovered branches
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 5. parquetValueToAny (storage_query.go) -- covered
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 6. parquetValueToInterface (reader_projected.go ~163) -- covered
// ---------------------------------------------------------------------------
...
func TestMedium_rowGroupMatchesTimeRange(t *testing.T) {
	...
	rows := []pushdownTestRow{
		{TimestampUnixNano: 1000, SpanName: "a", SeverityText: "info", ServiceName: "svc"},
		{TimestampUnixNano: 2000, SpanName: "b", SeverityText: "warn", ServiceName: "svc"},
		{TimestampUnixNano: 3000, SpanName: "c", SeverityText: "error", ServiceName: "svc"},
	}
	path := writePushdownTestParquet(t, dir, rows)
	f := openTestParquet(t, path)
	rgs := f.RowGroups()
...
// ---------------------------------------------------------------------------
// 9. readMapColumnToBlockCols (reader_columnar.go ~224) -- covered
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 10. NewFooterCache (footer_cache.go ~35) -- cover uncovered init paths
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Additional: findColumnIndex -- exercise fallback
// ---------------------------------------------------------------------------

func TestMedium_findColumnIndex(t *testing.T) {
	dir := t.TempDir()
	rows := []pushdownTestRow{
		{TimestampUnixNano: 1000, SpanName: "hello", SeverityText: "info", ServiceName: "svc"},
	}
	path := writePushdownTestParquet(t, dir, rows)
	f := openTestParquet(t, path)

	t.Run("existing column", func(t *testing.T) {
		idx := findColumnIndex(f.Root(), "span.name")
		if idx < 0 {
			t.Error("expected non-negative index for 'span.name'")
		}
	})
...
// ---------------------------------------------------------------------------
// Additional: mapColumnToAttrPrefix
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// partitionFromKey -- SKIPPED: function does not exist in traces module.
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// Additional: extractExactMatch
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Additional: extractFilterValues
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Additional: resolvePushDownIndices
// ---------------------------------------------------------------------------
	...
	t.Run("resolves existing columns", func(t *testing.T) {
		pdf := &PushDownFilter{
			Checks: []PushDownCheck{
				...
				{Column: "span.name", Op: PushDownPrefix, Value: "hel"},
			},
		}
...
// ---------------------------------------------------------------------------
// Additional: resolveBloomCheckIndices
// ---------------------------------------------------------------------------
...
func TestMedium_resolveBloomCheckIndices(t *testing.T) {
	...
	t.Run("resolves existing columns", func(t *testing.T) {
		checks := []bloomCheck{
			...
			{colName: "span.name", value: parquet.ValueOf("hello")},
		}
...
func TestMedium_allLeafColumns(t *testing.T) {
	...
	expected := []string{"timestamp_unix_nano", "span.name", "severity_text", "service.name"}
...
func TestMedium_traceRowToFields(t *testing.T) {
	...
	if _, ok := fieldMap["span_attr:http.method"]; !ok {
		t.Error("span_attr:http.method missing from fields")
	}
	if _, ok := fieldMap["span_attr:db.system"]; !ok {
		t.Error("span_attr:db.system missing from fields")
	}
...
// ---------------------------------------------------------------------------
// Additional: logRowToFields -- also present in traces module
// ---------------------------------------------------------------------------
