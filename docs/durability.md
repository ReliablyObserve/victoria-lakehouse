...
| `insert.ack_mode` | `buffer` | Designed as `buffer` (ack after the buffer add) or `flush-sync` (ack only after S3 confirms); no binary reads it in this release, so every insert is acknowledged once buffered. |
