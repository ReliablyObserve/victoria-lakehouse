...
All `stats` and `ui` keys (none of them has a flag; set them in the config file):

### Stats Config

<!-- BEGIN GENERATED: config-keys stats.enabled stats.push_interval stats.push_compression stats.snapshot_interval stats.snapshot_prefix stats.node_meta_ttl stats.meta_bucket stats.max_delta_count stats.metrics_cardinality_limit stats.cardinality_warning_threshold stats.breakdown_labels stats.s3_lifecycle_rules stats.s3_price_per_gb stats.s3_request_prices stats.s3_inventory_bucket stats.headobject_sample_interval stats.headobject_max_per_refresh -->
<!-- Generated by `make config-docs` from the code; do not edit. -->

| Key | Default | Config file | Flags | Description |
|---|---|---|---|---|
| `stats.enabled` | `true` | enable-only |  | Collects tenant statistics and syncs them across the fleet. |
| `stats.push_interval` | `30s` | set |  | How often stat deltas are broadcast to peers. |
| `stats.push_compression` | `true` | enable-only |  | Zstd-compresses delta broadcasts. |
| `stats.snapshot_interval` | `5m` | set |  | How often the full registry snapshot is written to S3. |
| `stats.snapshot_prefix` | `_meta/tenant-stats` | set |  | The S3 key prefix of the registry snapshot. |
| `stats.node_meta_ttl` | `1m30s` | set |  | Bounds how long a peer's gossiped metadata footprint stays in the fleet view (/stats/instances + the cluster-wide Overview sum) without a refresh. |
| `stats.meta_bucket` | `""` | set |  | A dedicated bucket for metadata in bucket isolation; empty uses s3.bucket. |
| `stats.max_delta_count` | `1000` | set |  | **Not read.** The number of deltas after which a full sync is forced. |
| `stats.metrics_cardinality_limit` | `100` | set |  | Caps distinct tenant label values in metrics. |
| `stats.cardinality_warning_threshold` | `10000` | set |  | **Not read.** The field cardinality that raises a high-cardinality warning. |
| `stats.breakdown_labels` | `[deployment.environment, service.name, k8s.namespace.name, k8s.cluster.name, k8s.deployment.name]` | ignored |  | The dimensions offered by the stats breakdown. |
| `stats.s3_lifecycle_rules` | `[]` | set |  | The bucket lifecycle rules used to predict storage classes without API calls. |
| `stats.s3_price_per_gb` | `{"DEEP_ARCHIVE": 0.00099, "GLACIER": 0.0036, "GLACIER_IR": 0.004, "STANDARD": 0.023, "STANDARD_IA": 0.0125}` | set |  | The storage price per GB-month of each storage class, for cost estimates. |
| `stats.s3_request_prices` | `{"GET": 0.0004, "LIST": 0.005, "PUT": 0.005}` | set |  | The price per 1000 requests of each request type, for cost estimates. |
| `stats.s3_inventory_bucket` | `""` | set |  | **Not read.** An S3 Inventory bucket used to verify storage classes exactly. |
| `stats.headobject_sample_interval` | `6h` | set |  | **Not read.** The interval of HeadObject spot checks near lifecycle transitions. |
| `stats.headobject_max_per_refresh` | `50` | set |  | **Not read.** Caps HeadObject calls per refresh. |

<!-- END GENERATED: config-keys -->

### UI Config

<!-- BEGIN GENERATED: config-keys ui.enabled ui.vmui_tab ui.refresh_default ui.theme -->
<!-- Generated by `make config-docs` from the code; do not edit. -->

| Key | Default | Config file | Flags | Description |
|---|---|---|---|---|
| `ui.enabled` | `true` | enable-only |  | Serves the Lakehouse Explorer at /lakehouse/ui/. |
| `ui.vmui_tab` | `true` | enable-only |  | Adds a Lakehouse tab to the VictoriaLogs/VictoriaTraces VMUI. |
| `ui.refresh_default` | `0` | set |  | **Not read.** The default auto-refresh interval in seconds; 0 disables it. |
| `ui.theme` | `auto` | set |  | **Not read.** The UI theme: auto, dark or light. |
