...
- **Field and stream enumeration.** `field_names`, `field_values` and `streams` read only the tenant's objects. `field_values` answers from the label counts of the tenant's own manifest entries or scans the tenant's objects; the pmeta catalog behind `field_names` is keyed by the tenant partition (the full key directory), so a catalog answer unions only the tenant's partitions. `field_values` never answers from the in-memory label index (its values are a sample and not time-scoped), and neither endpoint answers from it for a window holding none of the tenant's objects. That index is not tenant-keyed — it holds the field names of every object it was built from — so the `field_names` fallback that still reads it for a window that does hold objects (logs: when their footers cannot be read; traces: when the catalog has no names for the window) answers only a request for the one tenant the manifest holds objects for (objects under the legacy untenanted layout count as tenant `0:0`, so a default-tenant deployment part-way through adopting the prefix template keeps the fast path). Every other request — another tenant, an unknown tenant, `0:0` in a deployment whose only tenant is someone else, or a multi-tenant list — uses the tenant-partitioned catalog or a scan.
