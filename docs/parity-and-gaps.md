...
| **TraceQL search `/select/tempo/api/search?q=...`** | Broken | Functional-degradation | Phase 1 (`getTraceIDList`) works on cold (returns trace IDs). Phase 2 (`trace_id:in(...)` to fetch spans) returns 0 rows via the Tempo handler's DataBlock writeBlock callback, even though `stats count()` over the same filter returns the expected count. Smells like a `_time` typed-timestamp column issue: the handler calls `db.GetTimestamps(nil)` and aborts on `!ok`. Cold-tier reader may emit `_time` as a string field rather than a typed timestamp column, breaking the DataBlock contract this specific handler relies on. ~2-4h to fix in `reader_columnar.go` typed-column emission. Workaround: use Jaeger search (`/select/jaeger/api/traces`) which works fine on cold. |
