...
lakehouse-logs --lakehouse.role=select \
  --lakehouse.s3.bucket=obs-archive \
  --lakehouse.config=/etc/lakehouse/select.yaml
```

Select pods find the insert pods through peer discovery. `discovery.peer_headless_service`
has no flag, so it goes in the select pods' config file:

```yaml
# /etc/lakehouse/select.yaml
lakehouse:
  discovery:
    peer_headless_service: lakehouse-insert.monitoring.svc.cluster.local
```
...
```bash
lakehouse-logs --lakehouse.config=/etc/lakehouse/config.yaml
```

Flags are applied after the file. Not every key merges the same way — see
[How a setting is resolved](configuration.md#how-a-setting-is-resolved) for the exact rules.

## Configuration Profiles

Victoria Lakehouse ships with five configuration profiles — named sets of overrides on
top of the built-in defaults for a specific operational goal. Any key set in the config
file is merged over the profile value (see
[How a setting is resolved](configuration.md#how-a-setting-is-resolved)).
...
Every override of every profile is listed in
[Configuration — Configuration profiles](configuration.md#configuration-profiles).

### When to Use Each Profile

- **`balanced`**: The built-in defaults. Good trade-offs everywhere, with compaction, GC and stats on.
...
- **`max-durability`**: Retention on, GC every hour, higher zstd level for fresh writes. It also sets `insert.ack_mode: flush-sync` and hardened S3 retries, but no binary reads those keys in this release, so inserts are still acknowledged once buffered. For crash durability use the `logstore` buffer engine; see [Write Path Durability](./cross-az-optimization.md#write-path-durability--az-failure-protection).
...
- **`dev`**: For local development with MinIO. Tiny footprint, 1s flush, `force_path_style: true` for MinIO. GC, stats, compaction, and retention all disabled. Not for production.
...
`--lakehouse.profile` also exists, but it is applied under the already-loaded
configuration, so most of a profile's overrides do not take effect that way (see
[Profile flag gaps](configuration.md#profile-flag-gaps)). The Helm chart passes profiles
as that flag.

### Overriding Profile Settings

A key set in the config file overrides the profile value, as long as it is non-zero (and,
for `enable-only` booleans, `true`):

```yaml
lakehouse:
  profile: max-performance
  insert:
    buffer_engine: logstore     # durable on-disk buffer
    buffer_flush_enabled: true  # make the logstore buffer the authoritative Parquet producer
    compression_level: 7        # prefer space savings over speed
```
...
Logs insert uses `max-durability` (retention on, hourly GC, zstd level 7). Logs select uses
...
| `max-durability` | ~$570/yr | ~$4/yr | Standard | 19 MB per pod (`ack_mode` is not read) |
...
Compaction is on by default: it merges small L0 flush files into larger L1/L2 files, which
improves query performance over time by reducing file count and improving row-group
density. Every pod runs the scheduler, and HRW ownership assigns each partition to exactly
one pod — no leader election, no RBAC. Tune the thresholds in the config file, or the scan
interval with `--lakehouse.compaction.interval`.
...
- [Configuration Reference](configuration.md) — every key and flag with its default, generated from the code
