...
| **Data loss window** | Hot: 0 (VL replication)<br>Cold: **ack_mode dependent** — `buffer`: flush-interval (10s default), `flush-sync`: **zero** (S3 confirms before 200 OK), `wal`: zero for pod crash, flush-interval for AZ failure. See [Write Path Durability](./cross-az-optimization.md#write-path-durability--az-failure-protection). | flush-interval (10s default) | Ingester WAL flush: 1-5s<br>RF < 3: ingester death = data loss | Same as Loki | 0 (sync write) |
...
|---|---|---|---|---|
| **Replication needed?** | **No** — S3 provides multi-AZ replication inherently. Select pods are stateless (any replica count). Insert pods buffer only flush-interval of data. With `ack_mode=flush-sync`, zero data loss without any replication — S3 PutObject is confirmed before client gets 200 OK. See [Write Path Durability](./cross-az-optimization.md#write-path-durability--az-failure-protection). | **Yes** — RF=3 required for WAL durability. 3 ingester replicas write to EBS before S3 flush. | **Yes** — RF=3 required, same as Loki | **Yes** — VL `-replicationFactor` across AZs |
...
| **Deduplication needed?** | **No** — each insert pod writes unique time-partitioned files. S3 PutObject is atomic (no partial writes). A crash re-flush may write duplicate objects but the manifest prevents double-counting (and trace spans dedup by id). | **Yes** — ingester restarts can replay WAL, producing duplicate chunks. Compactor deduplicates. | **Yes** — same WAL replay issue | **No** — VL handles internally |
...
|---|---|---|---|---|
| **Insert pod crashes** | **Depends on durability config**: `buffer_engine: buffer` — max flush-interval data loss (`ack_mode: flush-sync` is designed for zero loss but not read by the binaries in this release). `buffer_engine: logstore` — loss ≤ buffer flush interval (~5s); on-disk parts restored + watermark re-flushes on restart, no WAL. Other pods always unaffected. | WAL replay on restart. RF=3 means 2 other replicas have the data. | Same as Loki | VL handles via replication |
...
| **Durability before S3** | **Buffer engine dependent**: `buffer` — flush-interval in memory. `logstore` — on-disk parts on the pod's volume, restored on restart. (`ack_mode: flush-sync`, 200 only after S3 confirms, is not read by the binaries in this release.) | WAL on single-AZ EBS. RF=3 ingesters replicate to survive single ingester loss. | Same as Loki — WAL + RF=3 replication. |
...
| **Write availability** | S3 is 99.99% available. Single insert pod failure: `buffer` engine — up to a flush interval of data; `logstore` engine — up to the buffer flush interval (~5s). Other pods unaffected (no shared state). | RF=3 means 1 ingester failure is tolerated. 2+ failures in same RF group = write rejection. Cross-AZ WAL replication needed. | Same as Loki. |
| **Recovery** | Restart pod → logstore buffer restores its on-disk parts → watermark re-flushes the uncommitted window (idempotent, no WAL). No coordination with other pods. | Restart → WAL replay → re-register with ring. May produce duplicate chunks requiring compaction. | Same as Loki. |
