...
|---|---|---|---|---|
| **Insert pod crashes** | **Depends on ack_mode**: `buffer` — max flush-interval (10s) data loss. `flush-sync` — **zero loss** (200 never sent, client retries). `wal` — zero loss (WAL replayed on restart). Other pods always unaffected. | WAL replay on restart. RF=3 means 2 other replicas have the data. | Same as Loki | VL handles via replication |
