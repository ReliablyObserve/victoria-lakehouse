...
| **Durability before S3** | **ack_mode dependent**: `buffer` — flush-interval in memory (10s default). `flush-sync` — **zero risk** (client only gets 200 after S3 PutObject confirms). `wal` — on local EBS, survives pod crash. | WAL on single-AZ EBS. RF=3 ingesters replicate to survive single ingester loss. | Same as Loki — WAL + RF=3 replication. |
...
| **Write availability** | S3 is 99.99% available. Single insert pod failure: `buffer` — max 10s data gap; `flush-sync` — zero gap (client retries). Other pods unaffected (no shared state). | RF=3 means 1 ingester failure is tolerated. 2+ failures in same RF group = write rejection. Cross-AZ WAL replication needed. | Same as Loki. |
